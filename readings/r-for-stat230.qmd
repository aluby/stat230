---
title: Using R/RStudio for Stat230
format: html
---

In Stat230, I will distribute homework assignment templates in.qmd format. This is called a "quarto" file and allows you to use written text, code, and math in a single document. To work on our quarto files, we'll use RStudio.

# Options for using RStudio

1. If you have taken other Stat courses at Carleton, you may have downloaded a local version of R/RStudio on your own computer. You are welcome to use that if you are comfortable installing packages and troubleshooting your file locations! 

2. Otherwise, you can access a browser-based version of RStudio at <https://maize.mathcs.carleton.edu>. 

The rest of this guide assumes you can access and login to maize through your Carleton credentials, or that you have a local version of R/RStudio functioning properly.
...
- The "editor" shows you the file you are currently working on. This is where you should edit your homework assignments or in-class activities and save as you go
- The "console" is where R code is run. You can use it like a calculator for short computations (like using `qnorm` to find a critical value or `pnorm` to find a probability), but I recommend using the editor with a file for any data analysis or model fitting 
...
![](images/rstudio-overview.png){fig-alt="A screenshot of rstudio showing the editor, console, environment, and files panes."}
...
![](images/knit-to-pdf.png){fig-alt="A screenshot showing the option to knit to PDF within rstudio."}
...
![](images/download-pdf.png){fig-alt="A screenshot with an arrow pointing to the button to download file in the maize PDF viewer."}

- If you'd like to make changes, just close the PDF and return to your .rmd or .qmd and repeat these steps


::: callout-tip
R and LaTeX are very picky when it comes to rendering PDF documents. If you copy and paste formatted text into your .qmd file, for example, it won't render and you'll get a bunch of error messages. Please don't copy and paste anything directly into your .qmd files! 
:::

::: callout-tip
If you do get errors, you can try knitting or rendering to a word document (.docx) or html (.html) and then saving that to PDF. Remember that any time you knit, you will re-write the file, so any changes you make in your .docx file will *not* be saved! 
:::
